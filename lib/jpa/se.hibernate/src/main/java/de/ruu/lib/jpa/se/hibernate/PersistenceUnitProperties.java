...
public class PersistenceUnitProperties
{
	public enum HBM2DLLAuto
	{
		...
		DROP       ("drop"),
		CREATE     ("create"),      // on startup performs drop, then create
		CREATE_DROP("create-drop"), // on startup performs drop, then create, on shutdown performs drop
		VALIDATE   ("validate"),
		UPDATE     ("update"),
		;
	...
	public Map<String, Object> properties()
	{
		...
		result.put(JAKARTA_JPA_JDBC_URL,     jdbcURL());
		...
		result.put(USE_REFLECTION_OPTIMIZER, useReflectionOptimizer());
		...
		if (jdbcDriver  != null) result.put(JAKARTA_JPA_JDBC_DRIVER, jdbcDriver.getName());
